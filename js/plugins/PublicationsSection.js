...
class PublicationsSection extends SectionPlugin {
  ...
  async init() {
    ...
    // Priority order:
    // 1. Pre-cached static data (if publications array exists with data)
    // 2. OpenAlex API (free, works from browser)
    // 3. Default empty state
    
    // Note: Scopus API does NOT support browser/client-side requests (CORS blocked)
    // To use Scopus data, run the fetch-scopus.js script locally and commit the results
    
    if (config.publications && config.publications.length > 0 && config.metrics) {
      // Use pre-cached static data (e.g., from Scopus script)
      this.publicationsData = config;
      this.dataSource = config.metrics.source || 'Cached';
      this._sortPublications();
    } else if (config.openAlexId || config.orcid) {
      // Fetch live from OpenAlex (works from browser, no API key needed)
  ...
  // ═══════════════════════════════════════════
  // OPENALEX API INTEGRATION (Browser-compatible)
  // ═══════════════════════════════════════════
  ...
  _transformOpenAlexData(authorData, worksData, config) {
    ...
    return {
      ...
      scopusProfile: config.scopusProfile || null,
